...
void HttpServer::start_single_acceptor() {
    if (!router_) {
        throw std::runtime_error("No router configured");
    }
    ...
    if (server->bind(options_.port) != 0) {
    ...
    LOG_INFO("Server listening on port ", options_.port, " with ", options_.num_workers, " workers");

    while (true) {
        auto* stream = server->accept();
        if (!stream) {
            LOG_WARN("Accept returned null");
            continue;
        }

        stream->timeout(30UL * 1000 * 1000); // 30 seconds
...
void HttpServer::start_multiple_acceptor() {
    ...
    for (size_t i = 0; i < options_.num_workers; ++i) {
        photon::thread_create11([this, i]() {
            this->worker_with_own_server(i);
        });
    }
...
void HttpServer::worker_with_own_server(size_t worker_id) {
    auto server = photon::net::new_tcp_socket_server();
    DEFER(delete server);
    ...
    if (server->bind(options_.port) != 0) {
        LOG_ERROR("Worker ", worker_id, " failed to bind to port ", options_.port);
        return;
    }

    if (server->listen() != 0) {
        LOG_ERROR("Worker ", worker_id, " failed to listen on port ", options_.port);
        return;
    }

    LOG_INFO("Worker ", worker_id, " listening on port ", options_.port);

    while (true) {
        auto* stream = server->accept();
        if (!stream) {
            LOG_WARN("Worker ", worker_id, " accept returned null");
            continue;
        }

        stream->timeout(30UL * 1000 * 1000);

        photon::thread_create11([this, stream, worker_id]() {
            DEFER(delete stream);
            this->handle_connection(stream);
        });
    }
        ...
        while (true) {
            ...
            if (parsed) {
                ...
                try {
                    ...
                    response = HttpResponse::internal_error("Internal Server Error");
                }
